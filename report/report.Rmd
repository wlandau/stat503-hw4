...
Figure \ref{fig:mc_pca} shows the principle component analysis. There are considerable overlappings between cluster 1 and 2, 2 and 3, and 4 and 5, suggesting rooms for improvement in terms of clustering. But we can still see patterns consistent with the parallel coordinates plot. 
...
```{r mc_predict, fig.cap="\\label{fig:mc_predict}Prediction of spring 2014 and fall 2014 clusters based on the equivalent mapping of homework topics by mapping the data onto the (modified) trained model.", fig.height=7}
