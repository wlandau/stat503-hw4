...
We also tried model based clustering to see if it will perform better. First we fit the model to fall 13. The VII specification with five clusters performs the best, which is shown in figure \ref{fig:mc_fall13}. As we can see it successfully picked out students who have relatively consistent performance in the upper range(cluster 1), and students who are relatively less stable throughout the semester(cluster 5).  
...
```
Figure \ref{fig:mean_mc_fall13} shows the mean of each assignment for each cluster. The 
